# utils file for refine cbf package
...
def define_constraint_function(obstacles: dict, padding: float):
    ...
    Returns:
        A constraint function based on the provided obstacles for use in Python HJ Reachability package. Takes current state as argument and
        returns a scalar (will be positive if the state is in the constraint set (i.e. outside an obstacle), negative or 0 otherwise).
    """
    ...
    def constraint_function(state):
        ...
        Returns:
            A scalar, positive iff the state is in not in the failure set, negative or 0 otherwise.
        """
